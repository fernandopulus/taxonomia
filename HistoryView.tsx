...
const HistoryView: React.FC<HistoryViewProps> = ({ analyses, onSelectAnalysis, onDeleteAnalysis }) => {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 bg-white shadow-xl rounded-lg">
      <h2 className="text-2xl font-bold text-indigo-700 mb-4">Historial de Análisis y Consolidados</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4 bg-slate-50 rounded-md shadow-sm">
        ...
        <div>
          ...
          <select
            ...
            className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm bg-white"
          >
      ...
      <div className="mb-6">
        <button 
          ...
          className="flex items-center justify-between w-full text-left px-4 py-3 bg-indigo-50 hover:bg-indigo-100 rounded-md text-indigo-700 font-semibold"
        >
        ...
        {showConsolidatedStats && (
          <div className="mt-2 p-4 border border-indigo-100 rounded-b-md">
        ...
        {filteredAnalyses.length > 0 ? (
          <ul className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
            {filteredAnalyses.map(analysis => (
              ...
              >
                <div>
                  <h4 className="font-semibold text-indigo-600">{analysis.instrumentTitle}</h4>
      ...
      <p className="mt-4 text-xs text-slate-500">
        Nota: Los datos se guardan localmente en su navegador. Limpiar los datos del navegador eliminará el historial.
